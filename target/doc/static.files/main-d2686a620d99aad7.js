...
function resourcePath(basename, extension) {
    return (
        getVar("root-path") + basename + getVar("resource-suffix") + extension
    );
}
function hideMain() {
...
(function () {
    ...
    window.searchState = {
        ...
        isDisplayed: () =>
            searchState.outputElement().parentElement.id ===
    ...
    window.register_implementors = (imp) => {
        ...
        const script = document.querySelector(
            "script[data-ignore-extern-crates]",
        );
        const ignoreExternCrates = script
            ? script.getAttribute("data-ignore-extern-crates")
            : "";
        for (const lib of libs) {
            if (
                lib === window.currentCrate ||
                ignoreExternCrates.indexOf(lib) !== -1
            ) {
                continue;
            }
    ...
    function addSidebarCrates() {
        ...
        const sidebarElems =
            document.getElementsByClassName("sidebar-elems")[0];
        if (!sidebarElems) {
            return;
        }
        const h3 = document.createElement("h3");
    ...
    (function () {
        ...
        function setImplementorsTogglesOpen(id, open) {
            ...
            if (list !== null) {
                onEachLazy(
                    list.getElementsByClassName("implementors-toggle"),
                    (e) => {
                        e.open = open;
                    },
                );
    ...
    window.rustdoc_add_line_numbers_to_examples = () => {
        onEachLazy(
            document.getElementsByClassName("rust-example-rendered"),
            (x) => {
                const parent = x.parentNode;
                const line_numbers = parent.querySelectorAll(
    ...
    window.addEventListener("resize", () => {
        if (
            window.innerWidth > window.RUSTDOC_MOBILE_BREAKPOINT &&
            oldSidebarScrollPosition !== null
        ) {
            hideSidebar();
        }
    ...
    function showNotable(e) {
        if (!window.NOTABLE_TRAITS) {
            ...
            if (data) {
                ...
                throw new Error(
                    "showNotable() called on page without any notable traits!",
                );
            }
        }
        if (
            window.CURRENT_NOTABLE_ELEMENT &&
        ...
        wrapper.innerHTML =
            '<div class="content">' + window.NOTABLE_TRAITS[ty] + "</div>";
        ...
        wrapper.onpointerleave = function (ev) {
            ...
            if (
                !e.NOTABLE_FORCE_VISIBLE &&
                !elemIsInParent(event.relatedTarget, e)
            ) {
                hideNotable(true);
            }
        };
    ...
    function notableBlurHandler(event) {
        if (
            ...
            !elemIsInParent(
                document.activeElement,
                window.CURRENT_NOTABLE_ELEMENT.NOTABLE_BASE,
            ) &&
            !elemIsInParent(
                event.relatedTarget,
                window.CURRENT_NOTABLE_ELEMENT.NOTABLE_BASE,
    ...
    onEachLazy(document.getElementsByClassName("notable-traits"), (e) => {
        e.onclick = function () {
            this.NOTABLE_FORCE_VISIBLE = this.NOTABLE_FORCE_VISIBLE
                ? false
                : true;
    ...
    function buildHelpMenu() {
        ...
        div_shortcuts.innerHTML =
            "<h2>Keyboard Shortcuts</h2><dl>" + shortcuts + "</dl></div>";
        const infos = [
    ...
    window.hidePopoverMenus = function () {
        onEachLazy(
            document.querySelectorAll(".search-form .popover"),
            (elem) => {
    ...
    if (isHelpPage) {
        showHelp();
        document
            .querySelector(`#${HELP_BUTTON_ID} > a`)
