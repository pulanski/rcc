...
(function () {
    ...
    function printTab(nb) {
        ...
        onEachLazy(
            document.getElementById("search-tabs").childNodes,
            (elem) => {
                if (nb === iter) {
    ...
    function initSearch(rawSearchIndex) {
        ...
        function getStringElem(query, parserState, isInGenerics) {
            if (isInGenerics) {
                ...
                throw new Error(
                    "Cannot have more than one literal search element",
                );
            } else if (parserState.totalElems - parserState.genericsElems > 0) {
                throw new Error(
                    "Cannot use literal search when there is more than one element",
                );
            }
            parserState.pos += 1;
            ...
            if (parserState.pos >= parserState.length) {
                ...
                throw new Error(
                    `Unexpected \`${parserState.userQuery[end]}\` in a string element`,
                );
            } else if (start === end) {
        ...
        function createQueryElement(
            ...
            if (
                ...
                throw new Error(
                    "You cannot have more than one element if you use quotes",
                );
            }
                ...
                if (!isIdentCharacter(c)) {
                    if (c === "!") {
                        if (foundExclamation) {
                            ...
                            throw new Error(
                                "`!` can only be at the end of an ident",
                            );
                        }
        ...
        function checkExtraTypeFilterCharacters(parserState) {
            ...
            for (let pos = 0; pos < parserState.pos; ++pos) {
                if (
                    !isIdentCharacter(query[pos]) &&
                    !isWhitespaceCharacter(query[pos])
                ) {
                    throw new Error(
                        `Unexpected \`${query[pos]}\` in type filter`,
                    );
                }
            }
        }
        ...
        function execQuery(
            ...
            function sortResults(results, isType, preferredCrate) {
                ...
                results.sort((aaa, bbb) => {
                    ...
                    if (
                        (aaa.item.ty === TY_PRIMITIVE &&
                            bbb.item.ty !== TY_KEYWORD) ||
                        (aaa.item.ty === TY_KEYWORD &&
                            bbb.item.ty !== TY_PRIMITIVE)
                    ) {
                        return -1;
                    }
                    if (
                ...
                if (parsedQuery.elems.length === 1) {
                    const hasPath =
                        typeof parsedQuery.elems[0].path === "undefined";
                ...
                for (const result of results) {
                    ...
                    if (
                        !isType &&
                        !validateResult(name, path, nameSplit, parent)
                    ) {
                        result.id = -1;
                    }
                }
            ...
            function checkGenerics(row, elem, defaultLev) {
                if (row.generics.length === 0) {
                    return elem.generics.length === 0
                        ? defaultLev
                        : MAX_LEV_DISTANCE + 1;
                } else if (
                    row.generics.length > 0 &&
                    row.generics[0].name === null
                ) {
                ...
                if (
                    ...
                    row.generics.length >= elem.generics.length
                ) {
                ...
                if (literalSearch) {
                    if (lev !== 0) {
                        if (elem.generics.length === 0) {
                            ...
                            if (
                                checkGeneric &&
                                row.generics.findIndex(
                                    (tmp_elem) => tmp_elem.name === elem.name,
                                ) !== -1
                            ) {
                                return 0;
                            }
            ...
            function findArg(row, elem, typeFilter) {
                ...
                if (
                    ...
                    for (const input of row.type.inputs) {
                        ...
                        lev = Math.min(
                            lev,
                            checkType(input, elem, parsedQuery.literalSearch),
                        );
                        if (lev === 0) {
                            return 0;
                        }
            ...
            function checkReturned(row, elem, typeFilter) {
                ...
                if (row && row.type && row.type.output.length > 0) {
                    ...
                    for (const ret_ty of ret) {
                        ...
                        lev = Math.min(
                            lev,
                            checkType(ret_ty, elem, parsedQuery.literalSearch),
                        );
                        if (lev === 0) {
                            return 0;
                        }
            ...
            function typePassesFilter(filter, type) {
                ...
                switch (itemTypes[filter]) {
                    ...
                    case "type":
                        return (
                            name === "primitive" || name === "associatedtype"
                        );
                    case "trait":
            ...
            function handleAliases(ret, query, filterCrates, currentCrate) {
                ...
                if (filterCrates !== null) {
                    if (
                        ...
                        ALIASES[filterCrates][lowerQuery]
                    ) {
                        const query_aliases = ALIASES[filterCrates][lowerQuery];
                        for (const alias of query_aliases) {
                            aliases.push(
                                createAliasFromItem(searchIndex[alias]),
                            );
                        }
                    }
                } else {
                    Object.keys(ALIASES).forEach((crate) => {
                        if (ALIASES[crate][lowerQuery]) {
                            const pushTo =
                                crate === currentCrate ? crateAliases : aliases;
            ...
            function addIntoResults(results, fullId, id, index, lev) {
                if (
                    lev === 0 ||
                    (!parsedQuery.literalSearch && lev <= MAX_LEV_DISTANCE)
                ) {
            ...
            function handleSingleArg(
                ...
                if (elem.fullPath.length > 1) {
                    ...
                    if (
                        lev > MAX_LEV_DISTANCE ||
                        (parsedQuery.literalSearch && lev !== 0)
                    ) {
                        return;
                    } else if (lev > 0) {
                        lev_add = lev / 10;
            ...
            function innerRunQuery() {
                ...
                if (parsedQuery.foundElems === 1) {
                    if (parsedQuery.elems.length === 1) {
                        ...
                        for (
                            i = 0, nSearchWords = searchWords.length;
                            i < nSearchWords;
                            ++i
                        ) {
                            handleSingleArg(
            ...
            handleAliases(
                ret,
                parsedQuery.original.replace(/"/g, ""),
                filterCrates,
                currentCrate,
            );
        ...
        function nextTab(direction) {
            const next =
                (searchState.currentTab + direction + 3) %
                searchState.focusedByTab.length;
        ...
        function showResults(results, go_to_first, filterCrates) {
            ...
            if (crates_list.length > 1) {
                ...
                for (const c of crates_list) {
                    crates += `<option value="${c}" ${
                        c === filterCrates && "selected"
                    }>${c}</option>`;
                }
                crates += "</select></div>";
            }
            ...
            return types.map((type) => {
                ...
                return {
                    name:
                        pathIndex === 0
                            ? null
                            : lowercasePaths[pathIndex - 1].name,
                    ty:
                        pathIndex === 0
                            ? null
                            : lowercasePaths[pathIndex - 1].ty,
                    generics: generics,
                };
            });
        }
            ...
            for (const crate in rawSearchIndex) {
                ...
                if (aliases) {
                    ...
                    for (const alias_name in aliases) {
                        ...
                        if (
                            !hasOwnPropertyRustdoc(ALIASES[crate], alias_name)
                        ) {
                            ALIASES[crate][alias_name] = [];
                        }
                        for (const local_alias of aliases[alias_name]) {
                            ALIASES[crate][alias_name].push(
                                local_alias + currentIndex,
                            );
                        }
                    }
                }
        ...
        function registerSearchEvents() {
            ...
            document.getElementsByClassName("search-form")[0].onsubmit =
                onSearchSubmit;
            ...
            searchState.input.addEventListener("blur", () => {
                searchState.input.placeholder =
                    searchState.input.origPlaceholder;
            });
            if (browserSupportsHistoryApi()) {
